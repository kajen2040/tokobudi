(async function(){const o=await axios.get("https://dummy-wrvf.vercel.app/location.json");$(".leaflet").each(async function(){const e=L.map(this,{center:[$(this).data("lat"),$(this).data("long")],zoom:9});if(e){const i=$(this).data("api-key");L.tileLayer(`https://tile.thunderforest.com/atlas/{z}/{x}/{y}@2x.png?apikey=${i}`,{attribution:"Map data &copy; OpenStreetMap contributors, Tiles &copy; Thunderforest"}).addTo(e);const n=L.markerClusterGroup({maxClusterRadius:30,iconCreateFunction:function(t){const a=$("html").hasClass("dark")?getColor("darkmode.100",.6):getColor("primary",.8),l=window.btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="55.066" height="47.691" viewBox="0 0 55.066 47.691">
                        ...
                        `);return L.divIcon({html:`<div class="relative w-full h-full">
                            ...
                            </div>`,className:"",iconSize:L.point(42,42),iconAnchor:L.point(20,45)})},spiderfyOnMaxZoom:!1,showCoverageOnHover:!1});e.addLayer(n);const r=$("html").hasClass("dark")?getColor("darkmode.100"):getColor("primary"),s=window.btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="31.063" viewBox="0 0 20 31.063">
                    ...
                    `);o.data.map(function(t){const a=L.marker({lat:parseFloat(t.latitude),lng:parseFloat(t.longitude)},{title:t.name,icon:L.icon({iconUrl:`data:image/svg+xml;base64,${s}`,iconAnchor:L.point(10,35)})});n.addLayer(a)})}})})();
